...
function Userprofile() {
  const { id } = useParams();
  const [provile, setProvile] = useState([]);
  useEffect(() => {
  ...
  return (
    <div className=" w-1/3 roounded overflow-hidden shadow-lg  mx-auto  p-5  ">
      <img
        className="w-50 h-50 mx-auto rounded-full mb-2"
        ...
        alt="card"
      ></img>
      <div className="text-2xl mx-auto font-serif text-cyan-800 text-center ">
        ID :{provile.id}
      </div>
      <div className="text-2xl mx-auto font-serif text-cyan-800 text-center">
        User Name:{provile.name}
      </div>
      <div className="text-2xl mx-auto font-serif text-cyan-800 text-center ">
        Email :{provile.email}
      </div>
      <div className="text-2xl mx-auto font-serif text-cyan-800 text-center ">
        gender:{provile.gender}
      </div>
      <div className="text-2xl mx-auto font-serif text-cyan-800 text-center mb-5">
        status:{provile.status}
      </div>
    </div>
  );
}
